...
extension Dictionary {
    ...
    func zx_signDic(_ insertMemberInfo:Bool = true) -> Dictionary<String,Any> {
        ...
        if insertMemberInfo {
            tempDic2["memberId"] = ZXUser.user.userId.zx_base64Encode()
        }
    ...
    func zx_signDicNotEncode() -> Dictionary<String,Any> {
        
        var tempDic = self as! Dictionary<String,Any>
        var token:String? = nil
        let userId = ZXUser.user.userId
        if let _token = tempDic["token"] as? String {
            token = _token
            tempDic.removeValue(forKey: "token")
        }
        tempDic["memberId"] = userId
