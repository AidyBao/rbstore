...
extension RBSearchRootViewController: UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if cellSize.width > ZXBOUNDS_WIDTH - 15*2 - 22 {
            cellSize = CGSize.init(width: ZXBOUNDS_WIDTH - 15*2 - 22, height: 22)
        }
        
        return CGSize.init(width: cellSize.width + 20, height: 22)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        ...
        case 1:
            if self.hotArray.count != 0 {
                size = CGSize.init(width: ZXBOUNDS_WIDTH, height: 40)
            }
        default:
            break
        }
...
extension RBSearchRootViewController: RBSearchHeaderDelegate {
    ...
    func didDeletedButtonAction(_ sender: UIButton) {
        ZXAlertUtils.showAlert(wihtTitle: "", message: "确定清空历史搜索吗？", buttonTexts: ["取消","确定"]) { (index) in
            switch index {
            case 1://清空历史记录
                self.requestForClearHistoryRecord()
            default:
                break
            }
        }
