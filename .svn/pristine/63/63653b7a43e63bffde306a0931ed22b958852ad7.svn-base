...
class ZXCartTableViewCell: MGSwipeTableCell {
    ...
    override func awakeFromNib() {
        ...
        self.swipeBackgroundColor = UIColor.white
    ...
    func reloadData(_ model:ZXCartModel,editing:Bool) {
        self.lbName.text = model.title
        self.lbSpec.text = "已选:\(model.specOptionValues)"
        ...
        if editing {
            self.btnCheck.isSelected = model.zx_editChecked
            self.reloadType(.editing)
        } else {
            self.btnCheck.isSelected = model.zx_isChosen
            self.reloadType(.normal)
        }
    }
    
    fileprivate func reloadType(_ ctype:ZXCartCellType) {
        self.cellType = ctype
        switch cellType {
        case .normal:
        ...
        case .editing:
            ...
            self.btnCheck.isHidden = false //编辑状态 无效 无货可以选择
        }
    }
