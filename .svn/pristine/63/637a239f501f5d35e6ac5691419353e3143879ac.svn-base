...
//                ZXRootController.selectedNav().pushViewController(activityList, animated: true)
            ...
            case .orderDetail:
                ZXUser.checkLogin({
                    if let mid = model?.showId,let sid = Int(mid) {
                        let orderDetail = RBOrderDetailRootController()
                        orderDetail.orderId = sid
                        ZXRootController.selectedNav().pushViewController(orderDetail, animated: true)
                    } else {
                        ZXHUD.showFailure(in: ZXRootController.appWindow()!, text: "订单ID丢失,请返回订单列表浏览", delay: ZXConst.zxdelayTime)
                    }
                })
            case .shoppingCart:
                ZXUser.checkLogin({
            ...
            case .floorWebPage:
                if let model = model as? ZXHomeFloorsModel {
                    ...
                    webPage.url = ZXAPI.activity(model.showId, type: .floorIds)
