...
class RBModifyTelViewController: ZXUITableViewController {
    ...
    @IBOutlet weak var getSecurityCodeBTN: ZXCountDownButton!
    @IBOutlet weak var securityCodeTextField: UITextField!
    var saveButton:UIButton?
    var tel: String = ""
    var telePhoneNum:String = ""
    var secCode:String = ""
    var lastTime: NSNumber = 0
    override func viewDidLoad() {
        ...
        //进入前台通知
        NotificationCenter.default.addObserver(self, selector: #selector(refreshDownTime), name: ZXNotification.UI.enterForeground.zx_noticeName(), object: nil)

    }
    
    func refreshDownTime() {
        if self.getSecurityCodeBTN.isCouting,ZXGlobalData.inoutCount > 0,self.getSecurityCodeBTN.currentCount > 0 {
            var temp = self.getSecurityCodeBTN.currentCount - ZXGlobalData.inoutCount
            temp = temp < 0 ? 0 : temp
            self.getSecurityCodeBTN.maxCount = temp
            self.getSecurityCodeBTN.start()
        }
    }
    
    func setUIStyle() -> Void {
        
        self.getSecurityCodeBTN.maxCount = 60
