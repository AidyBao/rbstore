...
//        self.tableView.register(UINib.init(nibName:String.init(describing: RBMessageContentCell.self), bundle: nil), forCellReuseIdentifier: RBMessageContentCell.RBMessageContentCellID)
        ...
        self.checkActivityBTN.isHidden = true
    ...
    func requestForDetailMessage() ->Void{
        ...
        ZXNetwork.asyncRequest(withUrl: ZXAPI.address(module: ZXAPIConst.Personal.messageDetail), params: params, method: .post) { (success, status, content, string, error) in
            if success {
                ...
                if let model = self.detailModel {
                    if model.zxValid {
                        self.checkActivityBTN.isEnabled = true
                        self.checkActivityBTN.setBackgroundImage(#imageLiteral(resourceName: "bg"), for: .normal)
                    } else {
                        self.checkActivityBTN.setBackgroundImage(nil, for: .normal)
                        self.checkActivityBTN.backgroundColor = .lightGray
                        self.checkActivityBTN.isEnabled = false
                    }
                    
                    self.checkActivityBTN.isHidden = false
                    self.checkActivityBTN.setTitle(model.zxStatusStr, for: .normal)
                }
