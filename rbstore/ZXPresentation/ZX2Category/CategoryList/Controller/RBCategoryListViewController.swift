...
class RBCategoryListViewController: ZXUIViewController {
    ...
    func filterAction() {
        ...
        if self.attributeList.count == 0 {
            ZXHUD.showText(in: self.view, text: "没有相关属性", delay: ZXHUD_MBDELAY_TIME)
            return
        }
...
//        self.toolMaskView.layer.shadowColor = UIColor.gray.cgColor
        self.toolMaskView.layer.shadowColor = UIColor.black.cgColor
        self.toolMaskView.layer.shadowRadius = 2
        self.toolMaskView.layer.shadowOffset = CGSize.init(width: 0, height: 0)
        self.toolMaskView.layer.shadowOpacity = 0.20
                ...
                if status == ZXAPI_SUCCESS {
                    if let data = content["data"] as? Dictionary<String,Any> {
                        ...
                        if let fieldList = data["fieldList"] as? Array<Any>,fieldList.count > 0 {
                            for dict in fieldList {
                                if ((dict as? Dictionary<String, Any>) != nil){
                                    let model: RBSortRootModel = RBSortRootModel.mj_object(withKeyValues: dict)
                                    self.attributeList.add(model)
                                }
                            }
                        }
                        ...
                        if let listData = data["productList"] as? Array<Any>,listData.count > 0 {
                            ...
                            if self.currentPageNum == 1 {
                                if tempArray.count != 0 {
                                    ...
                                    ZXHUD.showFailure(in: self.collectionView, text: "暂无相关商品", delay: ZXHUD_MBDELAY_TIME)
                                }
                            }else{
                                if tempArray.count != 0 {
                                    self.goodsArray.addObjects(from: tempArray as! [Any])
                                }else{
                                    ZXHUD.showFailure(in: self.collectionView, text: "没有更多商品了", delay: ZXHUD_MBDELAY_TIME)
                                }
                            }
                            ...
                            if self.currentPageNum == 1 {
                                ...
                                ZXEmptyView.show(in: self.collectionView, type: .noData, text: "暂无相关商品", retry: {
                                    ZXEmptyView.hide(from: self.collectionView)
                                    self.refreshForHeader()
                                })
                            }else{
